...
`双击脚本/` 目录提供三个可直接双击运行的脚本：

| 双击 | 行为 |
|---|---|
| `1-刷新IP钉选.bat` | 弹 UAC 后自动测速并更新 hosts 钉选 |
| `2-停止或恢复加速.bat` | 弹 UAC 后一键切换加速服务开 / 关 |
| `3-下载Release附件.bat` | 粘贴下载地址回车即下载（断点续传，存到 `下载/` 文件夹） |

`.bat` 是纯 ASCII 外壳（避免批处理中文乱码），实际逻辑在同目录的 `.ps1` 文件里，复制到任意目录双击即可用。
